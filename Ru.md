...
Нормальные показатели TPS это 19.8-20.0. В этой позиции держаться большинство Minecraft серверов. Если вы заметили, что всё начинает подлагивать и ваши игроки начинают жаловаться на проблемы, пропишите ```/tps``` и убедитесь, действительно ли сервер лагает. Также, консоль может выдавать предупреждение: Can't keep up! Is the server overloaded? Если это происходит - сразу приступайте к действиям.
...
### Оперативная память и использование процессора

У вас забивается оперативная память, а процессор грузится под 80+% при малой нагрузке? Вам нужно повышать мощности вашего сервера, ведь именно из-за нехватки кол-ва ядер или ГБ ОЗУ у вас могут возникать проблемы. Насчёт оперативной памяти: вам **обязательно** нужно поставить сборщик мусора при запуске сервера: -XX:+UseSerialGC.
По моему мнению, SerialGC самый лучший. Просто добавьте это в строку запуска сервера. Пример:
...
### Ядро
Рекомендую использовать [Airplane](https://github.com/TECHNOVE/Airplane) для 1.16-1.17.х. Для 1.12 и ниже - Spigot. **Не используйте** [Bukkit](https://getbukkit.org/), [Spigot](https://getbukkit.org/), [Paper](https://papermc.io/downloads) и другие, неизвестные ядра.
