...
08.07.21. Release: 1.1. Discord: cubelius#0988.
...
Для комфортной игры в 40 человек, вам понадобится 6 ГБ. Также, я рекомендую поставить [```hilltty-flags```](https://github.com/hilltty/hilltty-flags/blob/main/russian-lang.md). Эти флаги уменьшат потребление оперативной памяти, что очень нужно для сервера. Если у вас не будут стоять флаги, вы будете сталкиваться со многими проблемами. Флаги построены на алгоритме Shenandoah. Этот алгоритм намного лучше, чем тот, который применяется в флагах Айкара. 

Рекомендуемое кол-во ресурсов, чтобы потянуть определённое кол-во игроков и плагинов.
...
Самые популярные форки, которыми можно пользоваться на данный момент: [Paper](https://papermc.io/downloads#Paper-1.17), [Purpur](https://purpur.pl3x.net/downloads/#1.17), [Tuinity](https://ci.codemc.io/job/Spottedleaf/job/Tuinity/), [Airplane](https://github.com/TECHNOVE/Airplane). Не используйте Bukkit и Spigot, так как это пережитки прошлого, которые имеют много проблем. В Paper исправлено много багов и сделано много патчей, с производительностью всё прекрасно. На данный момент рекомендую использовать Airplane.
